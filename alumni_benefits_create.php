...
if (isset($_POST['submit'])) {
    // Handle photo upload
    $target_dir = "BCPAlumni-SMS3/benefitsphoto/";
    ...
    // SQL statement
    $sql = "INSERT INTO `bcp-sms3_alumnibenefits` (`title`, `photo`, `description`, `date`, `type`)
            VALUES ('$title', '$photo', '$description', '$date', '$type')";

    // Execute the query
    $result = mysqli_query($conn, $sql);
    if ($result) {  
        header("Location: alumni_benefits.php?msg=New record created successfully");
        exit(); 
    } else {
